...
2. <font color="#ffff00">(변경후) 업무 절차</font>
	1. 현업부서/작성자 (출장전) 업무
		...
		2. 근태코드 : 004(출장), 034(외근), 006(교육)
			1. 연차를 사용하는 방식과 동일하게 출장에 해당하는 근태원을 사전에 등록
		3. 근태원 접수
			1. 오프라인 : 근태원 출력물 결재 진행 / 접수
			2. 그룹웨어 : 그룹웨어내 휴가계(근태원) 서식을 이용하여, 결재 진행 / 접수
			>[!tip]
			>1. ERP 의 근태원 출력시, PDF로 저장하여 원본을 첨부합니다. 	
			>2. 그룹웨어 근태구분 선택시, ERP 근태원과 동일한 근태코드를 선택합니다.
			>3. 결재 편의를 위하여, 반드시 결재원본을 이미지 변환하여 내용에 첨부하여 결재를 상신합니다.
